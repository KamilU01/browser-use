...
class Agent(Generic[Context]):
    ...
    def _set_model_names(self) -> None:
        ...
        if hasattr(self.llm, "model_name"):
            ...
            hasattr(self.llm, "model_name")
            and "gemma" in str(self.llm.model_name).lower()
        ):
            self.model_name = str(self.llm.model_name)
    ...
    def _set_tool_calling_method(self) -> Optional[ToolCallingMethod]:
        ...
        if tool_calling_method == "auto":
            if (
                "deepseek-reasoner" in self.model_name
                or "deepseek-r1" in self.model_name
            ):
                return "raw"
            elif "gemma" in self.model_name.lower():
                return "raw"  # Gemma models typically work better with raw output
    ...
    async def get_next_action(self, input_messages: list[BaseMessage]) -> AgentOutput:
        ...
        converted_messages = convert_input_messages(input_messages, self.model_name)

        # Existing code for getting model response...
        # Use converted_messages instead of input_messages

        if self.tool_calling_method == "raw":
            output = self.llm.invoke(converted_messages)
            ...
            response: dict[str, Any] = await structured_llm.ainvoke(converted_messages)  # type: ignore
    ...
    async def take_step() -> tuple[bool, bool]:
        """Take a step
